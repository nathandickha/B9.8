...
function createCircularShape(radius, segments = CIRCULAR_SPA_SEGMENTS) {
  const shape = new THREE.Shape();
  shape.absarc(0, 0, radius, 0, Math.PI * 2, false);
  return shape;
}

function createCircularHole(radius, segments = CIRCULAR_SPA_SEGMENTS) {
...
function createCircularRingGeometry(outerRadius, innerRadius, height) {
  ...
  if (innerRadius > 0.005 && innerRadius < outerRadius - 0.005) {
    shape.holes.push(createCircularHole(innerRadius));
  }
  const geo = new THREE.ExtrudeGeometry(shape, {
    depth: Math.max(0.01, height),
    bevelEnabled: false,
    steps: 1,
    curveSegments: CIRCULAR_SPA_SEGMENTS
  });
  geo.computeVertexNormals();
  return geo;
}
...
function getSpaPlanDimensions(spa) {
  const length = spa?.userData?.spaLength ?? 2;
  const width = spa?.userData?.spaWidth ?? 2;
  const spaShape = spa?.userData?.spaShape || 'square';
  if (spaShape === 'circular') {
    const diameter = Math.max(1, Math.min(length, width));
    return { spaShape, length: diameter, width: diameter, radius: diameter * 0.5 };
  }
...
function updateSpaOutlinePoints(spa) {
  ...
  if (spaShape === 'circular') {
    const pts = [];
    for (let i = 0; i < CIRCULAR_SPA_SEGMENTS; i++) {
      const a = (i / CIRCULAR_SPA_SEGMENTS) * Math.PI * 2;
      pts.push(new THREE.Vector2(Math.cos(a) * radius, Math.sin(a) * radius));
    }
    spa.userData.outerPts = pts;
    return;
  }
  spa.userData.outerPts = [
    new THREE.Vector2(-length * 0.5, -width * 0.5),
    new THREE.Vector2(length * 0.5, -width * 0.5),
    new THREE.Vector2(length * 0.5, width * 0.5),
    new THREE.Vector2(-length * 0.5, width * 0.5)
  ];
...
function updateSpaSeats(spa) {
  ...
  if (spaShape === "circular") {
    ...
    if (seats.ring) {
      const outerR = Math.max(0.12, radius - 0.18);
      const innerR = Math.max(0.05, outerR - SPA_SEAT_DEPTH);
      const geo = createCircularRingGeometry(outerR, innerR, SPA_SEAT_THICKNESS);
      seats.ring.geometry.dispose();
      seats.ring.geometry = geo;
  ...
  if (seatTopAbs < bottom + 0.05) {
    ...
    [seats.front, seats.back, seats.left, seats.right, seats.ring].filter(Boolean).forEach((s) => {
...
function updateSpaWalls(spa) {
  ...
  spa.userData.spaLength = l;
  spa.userData.spaWidth = w;
  updateSpaOutlinePoints(spa);
  ...
  if (spaShape === "circular") {
    [walls.front, walls.right, walls.back, walls.left].forEach((wMesh) => { wMesh.visible = false; });
    if (walls.ring) {
      const outerR = radius + t * 0.5;
      const innerR = Math.max(0.05, radius - t * 0.5);
      const geo = createCircularRingGeometry(outerR, innerR, h);
      walls.ring.geometry.dispose();
      walls.ring.geometry = geo;
      walls.ring.position.set(0, 0, -h / 2);
      walls.ring.visible = true;
    }
    ...
    const waterRadius = Math.max(0.05, radius + 0.5 * (t + overflow));
    const newWaterGeo = createCircularDiscGeometry(waterRadius, 1);
    water.geometry.dispose();
    water.geometry = newWaterGeo;
    water.scale.set(1, 1, waterHeight);
    ...
    const poolWaterTop = SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST;
    const waterHeight = Math.max(0.01, poolWaterTop - waterBottomWorld);
    const newWaterGeo = new THREE.BoxGeometry(1, 1, 1);
    water.geometry.dispose();
    water.geometry = newWaterGeo;
    water.scale.set(
      l + 1.00 * (t + overflow),
  ...
  if (floor) {
    ...
    let geo;
    if (spaShape === "circular") geo = createCircularDiscGeometry(Math.max(0.05, radius - t), floorHeight);
    else {
      geo = new THREE.BoxGeometry(l, w, floorHeight);
      generateMeterUVsForBoxGeometry(geo, tileSize);
    }
...
export function createSpa(poolParams, scene, options = {}) {
  ...
  walls.ring.castShadow = true;
  walls.ring.receiveShadow = true;
  walls.ring.userData.isSpaWall = true;
  spa.add(walls.ring);
  ...
  seats.ring.castShadow = seats.ring.receiveShadow = true;
  seats.ring.userData.isSpaSeat = true;
  spa.add(seats.ring);
