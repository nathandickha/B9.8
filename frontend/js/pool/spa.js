...
function updateSpaSeats(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    if (seats.ring) { const outerR=Math.max(0.12,radius-0.18), innerR=Math.max(0.05,outerR-SPA_SEAT_DEPTH); seats.ring.geometry.dispose(); seats.ring.geometry=createCircularRingGeometry(outerR,innerR,SPA_SEAT_THICKNESS); seats.ring.position.set(0,0,seatCenterLocal-SPA_SEAT_THICKNESS/2); seats.ring.visible=true; }
  } else {
