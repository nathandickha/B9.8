...
function updateSpaWalls(spa) {
  ...
  if (spaShape === "circular") {
    ...
    if (walls.ring) {
      ...
      walls.ring.geometry = createCircularRingGeometry(radius + t * 0.5, Math.max(0.05, radius - t * 0.5), h);
    ...
    water.geometry = createCircularDiscGeometry(Math.max(0.05, radius + 0.5 * (t + overflow)), 1);
    water.scale.set(1, 1, waterHeight);
    water.position.set(0, 0, waterBottomWorld + waterHeight / 2 - spa.position.z);
    ...
    const waterHeight = Math.max(0.01, poolWaterTop - waterBottomWorld);
    water.geometry.dispose();
    water.geometry = new THREE.BoxGeometry(1, 1, 1);
    water.scale.set(l + 1.00 * (t + overflow), w + 1.00 * (t + overflow), waterHeight);
    water.position.set(0, 0, waterBottomWorld + waterHeight / 2 - spa.position.z);
    water.rotation.set(0, 0, 0);
  }
