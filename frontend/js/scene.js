...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  if (ENABLE_ORANGE_INTERIOR_DEBUG) {
    ...
    if (interiorClear) {
      ...
      if (spaGroup?.userData?.spaShape === 'circular') {
        const outerRadius = Math.max(0.01, (spaGroup.userData?.spaLength || 0.01) * 0.5);
        const innerRadius = Math.max(0.01, outerRadius - SPA_INTERIOR_CLEAR_WALL_INSET_XY - SPA_INTERIOR_CLEAR_EXTRA_INSET_XY);
        addDebugCircularDisc(group, innerRadius, new THREE.Vector3(interiorCenter.x, interiorCenter.y, interiorCenter.z), quat, SPA_INTERIOR_CLEAR_DEBUG_COLOR, 'SpaInteriorClearDebug');
      } else {
        addDebugBox(
          group,
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if (spaGroup?.userData?.spaShape === 'circular') {
    ...
    const copingOuterRadius = radius + Math.min(channelMargin, CIRCULAR_CHANNEL_COPING_WIDTH);
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      if (outside && arcStart === null) {
        ...
        if (arcEnd - arcStart > 0.01) {
          const band = createCircularArcBandShape(radius, outerRadius, arcStart, arcEnd);
          addCircularChannelExtrude(group, band, channelFloorThickness, channelFloorTopZ - channelFloorThickness, center, quat, mat);
          addCircularChannelExtrude(group, band, wallHeight, channelFloorTopZ, center, quat, mat);
          const copingBand = createCircularArcBandShape(radius, copingOuterRadius, arcStart, arcEnd);
