...
function createCircularChannelShape(innerRadius, outerRadius, snapSide = null) {
  const shape = new THREE.Shape(); shape.absarc(0, 0, Math.max(0.01, outerRadius), 0, Math.PI * 2, false);
  const inner = new THREE.Path(); inner.absarc(0, 0, Math.max(0.005, innerRadius), 0, Math.PI * 2, true); shape.holes.push(inner);
  const bridgeSide = snapSide === 'left' ? 'right' : snapSide === 'right' ? 'left' : snapSide === 'front' ? 'back' : snapSide === 'back' ? 'front' : null;
  if (bridgeSide) {
    const pad = 0.08; const slotHalf = outerRadius + pad; const slot = new THREE.Path();
    if (bridgeSide === 'right') { slot.moveTo(innerRadius - pad, -slotHalf); slot.lineTo(outerRadius + pad, -slotHalf); slot.lineTo(outerRadius + pad, slotHalf); slot.lineTo(innerRadius - pad, slotHalf); }
    else if (bridgeSide === 'left') { slot.moveTo(-outerRadius - pad, -slotHalf); slot.lineTo(-innerRadius + pad, -slotHalf); slot.lineTo(-innerRadius + pad, slotHalf); slot.lineTo(-outerRadius - pad, slotHalf); }
    else if (bridgeSide === 'back') { slot.moveTo(-slotHalf, innerRadius - pad); slot.lineTo(slotHalf, innerRadius - pad); slot.lineTo(slotHalf, outerRadius + pad); slot.lineTo(-slotHalf, outerRadius + pad); }
    else if (bridgeSide === 'front') { slot.moveTo(-slotHalf, -outerRadius - pad); slot.lineTo(slotHalf, -outerRadius - pad); slot.lineTo(slotHalf, -innerRadius + pad); slot.lineTo(-slotHalf, -innerRadius + pad); }
    slot.closePath(); shape.holes.push(slot);
  }
  return shape;
}
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if (spaGroup?.userData?.spaShape === 'circular') {
    ...
    const ringShape = createCircularChannelShape(radius, radius + channelMargin, snapSide);
    addCircularChannelExtrude(group, ringShape, channelFloorThickness, channelFloorTopZ - channelFloorThickness, center, quat, mat);
    addCircularChannelExtrude(group, ringShape, wallHeight, channelFloorTopZ, center, quat, mat);
    addCircularChannelExtrude(group, ringShape, copingDepth, copingUnderZ, center, quat, copingMat);
    const bridgeWidth = Math.max(0.35, radius * 1.2);
    const addBridge = (sx, sy, lx, ly) => { addChannelBox(group, sx, sy, channelFloorThickness, lx, ly, center, axisX, axisY, quat, mat, floorZCenter); addChannelBox(group, sx, sy, wallHeight, lx, ly, center, axisX, axisY, quat, mat, wallZCenter); addChannelBox(group, sx, sy, copingDepth, lx, ly, center, axisX, axisY, quat, copingMat, copingZCenter); };
    ...
    ground.parent?.add(group); if (ground.userData) ground.userData.spaChannelGroup = group; return;
  }
