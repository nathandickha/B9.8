...
function addDebugCylinder(group, radius, height, worldCenter, quat, color, name, opacity = 0.18, radialSegments = 96) {
  ...
  const bodyGeo = new THREE.CylinderGeometry(radius, radius, height, radialSegments, 1, false);
  bodyGeo.rotateX(Math.PI * 0.5);
  const body = new THREE.Mesh(bodyGeo, createDebugTransparentMaterial(color, opacity));
  body.position.copy(worldCenter);
  ...
  const topCap = new THREE.Mesh(capGeo, createDebugTransparentMaterial(color, opacity + 0.04));
  topCap.position.copy(worldCenter);
  topCap.position.z += height * 0.5;
  if (quat) topCap.quaternion.copy(quat);
  topCap.renderOrder = 1000;
  topCap.name = `${name}_topcap`;
  group.add(topCap);

  const bottomCap = new THREE.Mesh(capGeo.clone(), createDebugTransparentMaterial(color, opacity + 0.04));
  bottomCap.position.copy(worldCenter);
  bottomCap.position.z -= height * 0.5;
  bottomCap.rotation.x = Math.PI;
  if (quat) bottomCap.quaternion.copy(quat.clone().multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(Math.PI, 0, 0))));
  bottomCap.renderOrder = 1000;
  bottomCap.name = `${name}_bottomcap`;
  group.add(bottomCap);
...
function ensureOrangeInteriorClipMaterial(material) {
  ...
  material.customProgramCacheKey = () => {
    ...
    return `${prev}|orange-interior-clip-v2`;
  };
