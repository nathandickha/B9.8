...
function createCircularChannelShape(innerRadius, outerRadius, snapSide = null) {
  ...
  shape.absarc(0, 0, Math.max(0.01, outerRadius), 0, Math.PI * 2, false);

  const inner = new THREE.Path();
  inner.absarc(0, 0, Math.max(0.005, innerRadius), 0, Math.PI * 2, true);
  shape.holes.push(inner);

  const bridgeSide = snapSide === 'left' ? 'right'
    : snapSide === 'right' ? 'left'
    : snapSide === 'front' ? 'back'
    : snapSide === 'back' ? 'front'
    : null;
  ...
  if (bridgeSide) {
    const slotHalf = Math.max(0.18, innerRadius * 0.6);
    const pad = 0.08;
    const slot = new THREE.Path();
    if (bridgeSide === 'right') {
      slot.moveTo(innerRadius - pad, -slotHalf);
      slot.lineTo(outerRadius + pad, -slotHalf);
      slot.lineTo(outerRadius + pad, slotHalf);
      slot.lineTo(innerRadius - pad, slotHalf);
    } else if (bridgeSide === 'left') {
      slot.moveTo(-outerRadius - pad, -slotHalf);
      slot.lineTo(-innerRadius + pad, -slotHalf);
      slot.lineTo(-innerRadius + pad, slotHalf);
      slot.lineTo(-outerRadius - pad, slotHalf);
    } else if (bridgeSide === 'back') {
      slot.moveTo(-slotHalf, innerRadius - pad);
      slot.lineTo(slotHalf, innerRadius - pad);
      slot.lineTo(slotHalf, outerRadius + pad);
      slot.lineTo(-slotHalf, outerRadius + pad);
    } else if (bridgeSide === 'front') {
      slot.moveTo(-slotHalf, -outerRadius - pad);
      slot.lineTo(slotHalf, -outerRadius - pad);
      slot.lineTo(slotHalf, -innerRadius + pad);
      slot.lineTo(-slotHalf, -innerRadius + pad);
    }
...
function updateGroundMaterialSpaClip(ground, spaGroup = null, poolGroup = null) {
  ...
  uniforms.spaClipShape.value = isCircularSpa ? 1 : 0;
  uniforms.spaClipRadius.value = isCircularSpa
    ? Math.max(0.01, (spaGroup.userData?.spaLength || 0.01) * 0.5 + Math.max(margins.minX, margins.maxX, margins.minY, margins.maxY) - SPA_GROUND_CLIP_MARGIN_REDUCTION + pad)
